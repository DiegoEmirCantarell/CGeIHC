...
float dragonPosX = 5.5f; // Posición vertical del cartel
float cartelRotation = 0.05f;
float rotadragon = 0;// Rotación del cartel
float cartelSpeed = 0.05f;
float dragonSpeed = 0.2f;
int state = 0;
int state1 = 0;
...
int main()
{
	...
	{
		GLfloat time = glfwGetTime(); // Tiempo total
		GLfloat frequency = 1.0f;
		GLfloat now = glfwGetTime();
		deltaTime = now - lastTime;
		deltaTime += (now - lastTime) / limitFPS;
		lastTime = now;
		float Frecuencia = 2.0f; // Frecuencia del aleteo
		float Amplitud = 0.2f;  // Amplitud del aleteo
		...
		switch (state1) {
		case 0:
			dragonPosX -= dragonSpeed * deltaTime;
			if (dragonPosX <= -30.0f) {
				dragonPosX = -30.0f;
				rotadragon = 180;
				state1 = 1; 
		...
		model = glm::translate(model, glm::vec3(dragonPosX, 0.0f, 0.0f));
		model = glm::translate(model, glm::vec3(movCoche*5, 0.0f, 0.0f));
		model = glm::rotate(model, rotadragon * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		modelauxdragon = model;
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Dragon_M.RenderModel();

		// Aleteo para el ala izquierda
		model = modelauxdragon;
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.0f));
		model = glm::rotate(model, Amplitud * sin(Frecuencia * time), glm::vec3(0.0f, 0.0f, 1.0f)); // Aleteo en el eje X
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		alaizq.RenderModel();

		// Aleteo para el ala derecha
		model = modelauxdragon;
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.0f));
		model = glm::rotate(model, Amplitud * sin(Frecuencia * time), glm::vec3(0.0f, 0.0f, 1.0f)); // Aleteo en el eje X
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		// Cabeza 1: Movimiento en espiral de Arquímedes
		model = modelauxdragon;
		model = glm::translate(model, glm::vec3(-1.9f + 0.1f * time * cos(frequency * time),
			1.5f, // Mantener la posición inicial en Y
			0.5f + 0.1f * time * sin(frequency * time))); // Cambia la posición en Z
		color = glm::vec3(0.0f, 1.0f, 0.0f); // Verde
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		cabeza1.RenderModel();

		// Cabeza 2: Movimiento senoidal
		model = modelauxdragon;
		model = glm::translate(model, glm::vec3(-1.8f, 3.5f, // Mantener la posición inicial en Y
			0.5f + 0.5f * sin(frequency * time))); // Cambia la posición en Z
		color = glm::vec3(1.0f, 0.0f, 0.0f); // Rojo
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		cabeza2.RenderModel();

		// Cabeza 3: Movimiento en lemniscata
		model = modelauxdragon;
		model = glm::translate(model, glm::vec3(-2.0f + 0.5f * sin(2 * frequency * time),
			2.0f, // Mantener la posición inicial en Y
			-0.8f + 0.5f * sin(frequency * time) * cos(frequency * time))); // Cambia la posición en Z
		color = glm::vec3(0.0f, 0.0f, 1.0f); // Azul
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		cabeza3.RenderModel();

		// Cabeza 4: Movimiento vertical
		model = modelauxdragon;
		model = glm::translate(model, glm::vec3(-2.0f, 3.3f, // Mantener la posición inicial en Y
			-1.0f + 0.5f * sin(frequency * time))); // Cambia la posición en Z
		...
		model = modelauxdragon;
		model = glm::translate(model, glm::vec3(-2.2f + 0.5f * cos(frequency * time),
			2.3f, // Mantener la posición inicial en Y
			-0.8f + 0.5f * sin(frequency * time))); // Cambia la posición en Z
		color = glm::vec3(0.4f, 0.2f, 0.1f); // Marrón oscuro
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		cabeza5.RenderModel();
		...
		*/
		if (cambio <= (int)glfwGetTime())
		{
			...
			if (contanum == 1 && toffsetnumerocambiau >= 0.5)
			{
				toffsetnumerocambiau = 0.0;
				contanum = 0;
			}
			toffsetnumerov = 0.33 * contanum;
			cambio = (int)glfwGetTime() + 1;
		}
		...
		if (signum <= (int)glfwGetTime())
		{
			signum = (int)glfwGetTime() + 2;
			change = !change;
		}
